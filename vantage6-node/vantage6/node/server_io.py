...
class NodeClient(ClientBase):
    ...
    def set_task_start_time(self, id_: int) -> None:
        ...
        self.patch_results(id_, None, result={
            "started_at": datetime.datetime.now().isoformat()
        })

    def patch_results(self, id_: int, init_org_id: int, result: dict) -> dict:
        """
        ...
        init_org_id : int
            Organization ID of the origin of the task. This is required because
            we want to encrypt the results specifically for them
        result : dict
            The updated data to send to the server
        """
